...
export class AppEffects implements Effects<AppComponent> {
    ...
    countChange(state: State<TestState>) {
        return changes(state.count).subscribe(context(state).countChange)
    }
}
----
...
export class AppEffects {
    @Effect()
    handleTemplateClick(state: State<AppComponent>) {
        return context(state).clicked.subscribe(event => console.log(`click:`, event))
    }
}
...
export class AppEffects {
    ...
    @Effect({ whenRendered: true })
    mounted(state: State<AppComponent>) {
...
export class AppEffects {
    ...
    @Effect()
    dispatchForm(state: State<AppComponent>) {
        return context(state).formData.valueChanges.subscribe(payload => {
            this.store.dispatch({
                type: "FORM_UPDATED",
                payload
            })
    ...
    fetchUsers(state: State<AppComponent>) {
        ...
        ).subscribe(context(state).usersFetched)
    }
}
...
export class AppEffects {
    @Effect(Dispatch)
    dispatchAction(state: State<AppComponent>) {
        return context(state).formData.valueChanges.pipe(
            map(payload => ({
                type: "FORM_UPDATED",
                payload
            }))
