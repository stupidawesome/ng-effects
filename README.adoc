...
Use `HostEmitter` to bind `HostListener` properties. `HostEmitter` is a special subject that can be invoked as a function. It's value stays unwrapped in `State` for convenient access.
