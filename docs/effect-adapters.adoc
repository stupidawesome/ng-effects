...
Here we have a very simple effect adapter. If you're familiar with observers, you'll see how this works in a moment. Before using an effect effect adapter, we have to provide it in our app. In this case we have added it to the "root" provider.
...
Here we have a component that maps global `count` state to a local `count` variable inside the template of a component. We're already familiar with the problems that come with this approach, and we've two more.
